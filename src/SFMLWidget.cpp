...
SFMLWidget::SFMLWidget(sf::VideoMode mode, int size_request)
  ...
  set_has_window(false); // Makes this behave like an interal object rather than a parent window.
}
...
void SFMLWidget::on_realize() {
  ...
  if (!window) {
    ...
    window =
        Gdk::Window::create(get_window(), &attributes, GDK_WA_X | GDK_WA_Y);
    set_has_window(true);
    set_window(window);
    ...
    renderWindow.create(GET_WINDOW_HANDLE_FROM_GDK(window->gobj()));
  }
}
